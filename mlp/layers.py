...
class DropoutLayer(StochasticLayer):
    ...
    def fprop(self, inputs, stochastic=True):
        ...
        if stochastic:
            ...
            random_vector = self.rng.uniform(0, 1.0, size=mask_shape)
            mask = (random_vector < self.incl_prob).astype(inputs.dtype)
            self._mask = mask
            outputs = inputs * mask
        else:
    ...
    def bprop(self, inputs, outputs, grads_wrt_outputs):
        ...
        Returns:
            ...
            (batch_size, input_dim).
        """
        grads_wrt_inputs = grads_wrt_outputs * self._mask
